...
class EditProfileState extends State<EditProfile> {
  ...
  Future updateData() async {
    FirebaseFirestore.instance.collection("Users").doc(widget.currentUser.id).set(
      {'editInfo': editInfo, 'age': widget.currentUser.age},
    );
  }
  ...
  Future getImage(ImageSource imageSource, context, currentUser, isProfilePicture) async {
    ...
    if (image != null) {
      ...
      if (croppedFile != null) {
        await uploadFile(await compressimage(File(croppedFile.path)), currentUser, isProfilePicture);
      }
    }
    Navigator.pop(context);
  }
  ...
  Future uploadFile(File image, User currentUser, isProfilePicture) async {
    ...
    // if (uploadTask.snapshot.storage. == true) {}
                            ...
                            child: ClipRRect(
                              ...
                              child: Container(
                                decoration: widget.currentUser.imageUrl.length > index
                                    ...
                                    : BoxDecoration(
                                        borderRadius: BorderRadius.circular(10), border: Border.all(style: BorderStyle.solid, width: 1, color: secondryColor)),
                                child: Stack(
                                  children: <Widget>[
  ...
  void _deletePicture(index) async {
    ...
    await FirebaseFirestore.instance.collection("Users").doc("${widget.currentUser.id}").set(
      {"Pictures": temp[0]},
    );
  }
}
