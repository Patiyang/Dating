      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            RichText(
              textAlign: TextAlign.center,
              text: TextSpan(text: "Didn't receive the code? ", style: TextStyle(color: Colors.black54, fontSize: 15), children: [
                TextSpan(text: " RESEND", recognizer: onTapRecognizer, style: TextStyle(color: Color(0xFF91D3B3), fontWeight: FontWeight.bold, fontSize: 16))
              ]),
            ),
            SizedBox(
              height: 40,
            ...
            InkWell(
              ...
              onTap: () async {
                ...
                AuthCredential _phoneAuth = PhoneAuthProvider.credential(verificationId: widget.smsVerificationCode, smsCode: code);
                if (widget.updateNumber) {
                  User user =  FirebaseAuth.instance.currentUser;
                  user.updatePhoneNumber(_phoneAuth).then((_) => updateNumber()).catchError((e) {
                    CustomSnackbar.snackbar("$e", _scaffoldKey, context);
                  });
                } else {
                  FirebaseAuth.instance.signInWithCredential(_phoneAuth).then((authResult) {
                    ...
                    CustomSnackbar.snackbar("$onError", _scaffoldKey, context);
                  });
                }
              },
            )
